...
export class ActivitiesService implements OnApplicationBootstrap {
  ...
  async startListening() {
    ...
    this.activeWindowSubscriptionId = ActiveWindow.subscribe(async (windowInfo: WindowInfo | null) => {
      if (!windowInfo) {
        return;
      }
      ...
      if (!this.lastActivity) {
        ...
        (this.lastActivity.programName === currentActivity.programName && this.lastActivity.windowTitle === currentActivity.windowTitle) ||
        ...
        console.log('changed application or title: ', new Date().toISOString(), windowInfo?.application, windowInfo?.title);
        this.create({
  ...
  async create(activity: Activity): Promise<void> {
    ...
    await this.databaseService.db.run(
      'INSERT INTO activities (id, programName, windowTitle, startedAt, endedAt) VALUES ($id, $programName, $windowTitle, $startedAt, $endedAt)',
      params
    );
  }

  async findAll(startedAt: string, endedAt: string): Promise<Activity[]> {
    const results = await this.databaseService.db.all<Activity>(
      `
      ...
      WHERE activities.startedAt > $startedAt
        AND activities.endedAt < $endedAt
    `,
